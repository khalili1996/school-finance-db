...
@foreach($academicYears as $ay)
    <option value="{{ $ay->id }}"
        {{ old('academic_year_id', session('active_academic_year_id')) == $ay->id ? 'selected' : '' }}>
        {{ $ay->name }}
    </option>
@endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="month_id" id="month_id" class="form-select">
                            ...
                            @foreach($months as $m)
                                <option value="{{ $m->id }}" {{ old('month_id') == $m->id ? 'selected' : '' }}>{{ $m->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="base_salary" id="base_salary" class="form-control" value="{{ old('base_salary') }}" min="0" required>
                    </div>

                    {{-- اضافه‌کاری --}}
                    <div class="col-md-4 mb-3">
                        <label class="form-label">اضافه‌کاری</label>
                        <input type="number" name="overtime_amount" id="overtime_amount" class="form-control" value="{{ old('overtime_amount', 0) }}" min="0">
                    </div>

                    {{-- پاداش --}}
                    <div class="col-md-4 mb-3">
                        <label class="form-label">پاداش</label>
                        <input type="number" name="bonus_amount" id="bonus_amount" class="form-control" value="{{ old('bonus_amount', 0) }}" min="0">
                    </div>

                    {{-- کسورات --}}
                    <div class="col-md-4 mb-3">
                        <label class="form-label">کسری‌ها</label>
                        <input type="number" name="deduction_amount" id="deduction_amount" class="form-control" value="{{ old('deduction_amount', 0) }}" min="0">
                    </div>

                    {{-- ضمانت --}}
                    <div class="col-md-4 mb-3">
                        <label class="form-label">ضمانت</label>
                        <input type="number" name="guarantee_amount" id="guarantee_amount" class="form-control" value="{{ old('guarantee_amount', 0) }}" min="0">
                    </div>

                    {{-- مبلغ مالیات --}}
                    <div class="col-md-4 mb-3">
                        <label class="form-label">مبلغ مالیات (افغانی)</label>
                        <input type="number" name="tax_amount" id="tax_amount" class="form-control" value="{{ old('tax_amount', 0) }}" min="0" step="0.01">
                    </div>
