...
<div class="container-fluid px-4">
    ...
    <div class="card shadow">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('school.salaries.update', $salary->id) }}" method="POST">
                @csrf @method('PUT')
                <div class="row">
                    {{-- کارمند --}}
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="academic_year_id" class="form-select" required>
                            ...
                            @foreach($academicYears as $ay)
                                <option value="{{ $ay->id }}" {{ old('academic_year_id', $salary->academic_year_id) == $ay->id ? 'selected' : '' }}>{{ $ay->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="term_id" class="form-select">
                            ...
                            @foreach($terms as $term)
                                <option value="{{ $term->id }}" {{ old('term_id', $salary->term_id) == $term->id ? 'selected' : '' }}>{{ $term->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="month_id" id="month_id" class="form-select">
                            ...
                            @foreach($months as $m)
                                <option value="{{ $m->id }}" {{ old('month_id', $salary->month_id) == $m->id ? 'selected' : '' }}>{{ $m->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="base_salary" id="base_salary" class="form-control" value="{{ old('base_salary', $salary->base_salary) }}" min="0" required>
                    </div>

                    {{-- اضافه‌کاری --}}
                    <div class="col-md-4 mb-3">
                        <label class="form-label">اضافه‌کاری</label>
                        <input type="number" name="overtime_amount" id="overtime_amount" class="form-control" value="{{ old('overtime_amount', $salary->overtime_amount) }}" min="0">
                    </div>

                    {{-- پاداش --}}
                    <div class="col-md-4 mb-3">
                        <label class="form-label">پاداش</label>
                        <input type="number" name="bonus_amount" id="bonus_amount" class="form-control" value="{{ old('bonus_amount', $salary->bonus_amount) }}" min="0">
                    </div>

                    {{-- کسورات --}}
                    <div class="col-md-4 mb-3">
                        <label class="form-label">کسورات</label>
                        <input type="number" name="deduction_amount" id="deduction_amount" class="form-control" value="{{ old('deduction_amount', $salary->deduction_amount) }}" min="0">
                    </div>

                    {{-- ضمانت --}}
                    <div class="col-md-4 mb-3">
                        <label class="form-label">ضمانت</label>
                        <input type="number" name="guarantee_amount" id="guarantee_amount" class="form-control" value="{{ old('guarantee_amount', $salary->guarantee_amount) }}" min="0">
                    </div>

                    {{-- مبلغ مالیات --}}
                    <div class="col-md-4 mb-3">
                        <label class="form-label">مبلغ مالیات (افغانی)</label>
                        <input type="number" name="tax_amount" id="tax_amount" class="form-control" value="{{ old('tax_amount', $salary->tax_amount) }}" min="0" step="0.01">
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="status" class="form-select" required>
                            <option value="unpaid" {{ old('status', $salary->status) == 'unpaid' ? 'selected' : '' }}>پرداخت نشده</option>
                            <option value="partially_paid" {{ old('status', $salary->status) == 'partially_paid' ? 'selected' : '' }}>پرداخت جزئی</option>
                            <option value="paid" {{ old('status', $salary->status) == 'paid' ? 'selected' : '' }}>پرداخت کامل</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" id="advance_sum_display" class="form-control" readonly value="{{ $currentAdvanceSum }}">
                    </div>
                    ...
                    <div class="col-12 mb-3">
                        ...
                        <textarea name="notes" class="form-control" rows="2">{{ old('notes', $salary->notes) }}</textarea>
                    </div>
                </div>
...
const employeesPoints = {
    @foreach($employees as $emp)
        {{ $emp->id }}: {
            position_points: {{ $emp->position_points ?? 0 }},
            experience_points: {{ $emp->experience_points ?? 0 }},
            education_points: {{ $emp->education_points ?? 0 }}
        },
    @endforeach
};
