...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex gap-2">
        <a href="{{ route('school.employees.create') }}" class="btn btn-success">
            <i class="fas fa-plus"></i> ثبت کارمند جدید
        </a>
        <a href="{{ route('school.employees.trash') }}" class="btn btn-outline-danger">
            <i class="fas fa-trash-alt"></i> سطل زباله
            ({{ \App\Models\Employee::onlyTrashed()->where('school_id', session('active_school_id', auth()->user()->school_id))->count() }})
        </a>
    </div>
</div>
    ...
    <form method="GET" class="row g-2 mb-4">
        <div class="col-md-2"><input type="text" name="search" class="form-control" placeholder="جستجوی نام، کد، تماس..." value="{{ request('search') }}"></div>
        <div class="col-md-2">
        ...
        <div class="col-auto">
            ...
            <a href="{{ route('school.employees.index') }}" class="btn btn-secondary">حذف فیلترها</a>
        </div>
    </form>

    {{-- انتقال گروهی --}}
    @if(isset($nextYears) && $nextYears->isNotEmpty())
    <form method="POST" action="{{ route('school.employees.transfer-multiple') }}" id="bulkTransferForm" class="mb-3">
        @csrf
                ...
                <tbody>
                    @forelse($employees as $emp)
                        <tr>
                            ...
                            <td>{{ $emp->phone ?? '—' }}</td>
                            <td><span class="badge bg-{{ $emp->status == 'active' ? 'success' : 'danger' }}">{{ $emp->status == 'active' ? 'فعال' : 'غیرفعال' }}</span></td>
                            <td>{{ $emp->hire_date ? \App\Helpers\JalaliHelper::toJalali($emp->hire_date) : '—' }}</td>
                            <td>
                                <a href="{{ route('school.employees.show', $emp->id) }}" class="btn btn-sm btn-info"><i class="fas fa-eye"></i></a>
                                <a href="{{ route('school.employees.edit', $emp->id) }}" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                <form action="{{ route('school.employees.destroy', $emp->id) }}" method="POST" class="d-inline" onsubmit="return confirm('حذف شود؟')">
                                    @csrf @method('DELETE')
                                ...
                                {{-- انتقال تکی --}}
                                @if(isset($nextYears) && $nextYears->isNotEmpty())
                                <form method="POST" action="{{ route('school.employees.transfer-single', $emp) }}" class="d-inline">
                                    @csrf
                                    <input type="hidden" name="target_year_id" value="">
                                    <button type="button" class="btn btn-sm btn-outline-primary transfer-btn" data-employee-name="{{ $emp->first_name }} {{ $emp->last_name }}">
