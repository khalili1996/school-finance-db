...
<head>
    <meta charset="UTF-8">
    <title>گزارش دانش‌آموزان – {{ $selectedClass ?? 'کل مکتب' }}</title>
...
<body>
    ...
    <div class="header">
        <h2>لیست دانش‌آموزان – {{ $selectedClass ?? 'کل مکتب' }}</h2>
        <h4>تعداد کل: {{ $students->count() }} | تاریخ: {{ now()->format('Y/m/d') }}</h4>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>پدرکلان</th>
                <th>نمبر اساس</th>
                @if(!$selectedClass)<th>صنف</th>@endif
                <th>شماره تماس</th>
                <th>شهریه کل</th>
                <th>پرداختی</th>
                <th>باقی‌مانده</th>
        ...
        <tbody>
            @php $totalFeeAll = 0; $totalPaidAll = 0; @endphp
            @foreach($students as $student)
                ...
                <tr>
                    ...
                    <td>{{ $student->base_number ?? '—' }}</td>
                    @if(!$selectedClass)<td>{{ $student->class ?? '—' }}</td>@endif
                    <td>{{ $student->phone ?? '—' }}</td>
                    <td>{{ number_format($totalFee) }}</td>
                    <td>{{ number_format($totalPaid) }}</td>
                    <td>{{ number_format($balance) }}</td>
                    <td>{{ implode('، ', $student->unpaidMonths) ?: '—' }}</td>
                </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr style="background:#f0f0f0; font-weight:bold;">
                <td colspan="{{ $selectedClass ? '6' : '7' }}">مجموع</td>
                <td>{{ number_format($totalFeeAll) }}</td>
                <td>{{ number_format($totalPaidAll) }}</td>
                <td>{{ number_format($totalFeeAll - $totalPaidAll) }}</td>
                <td></td>
            </tr>
        </tfoot>
    </table>
