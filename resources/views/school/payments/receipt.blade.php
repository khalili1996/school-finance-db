...
<div class="receipt-container">
    ...
    <table class="info-table">
        <tr>
            ...
            <td class="value">{{ $payment->receipt_number ?? $payment->id }}</td>
        </tr>
        <tr>
            <td class="label">تاریخ و ساعت</td>
            <td class="value">@jalali($payment->payment_date) | {{ $payment->created_at->format('H:i') }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="value">{{ $payment->student->father_name }}</td>
        </tr>
        <tr>
            <td class="label">نمبر اساس</td>
            <td class="value">{{ $payment->student->base_number ?? '—' }}</td>
        </tr>
        <tr>
            <td class="label">صنف</td>
            <td class="value">{{ $payment->student->class ?? '—' }}</td>
        </tr>
        @if($payment->studentFee)
        <tr>
            <td class="label">ماه</td>
            <td class="value">{{ $payment->studentFee->month->name ?? '—' }}</td>
        </tr>
        @endif
        <tr>
            <td class="label">شهریه تعیین شده</td>
            <td class="value">{{ number_format($totalFee) }} ؋</td>
        </tr>
        <tr>
            <td class="label">مبلغ پرداختی</td>
            <td class="value">{{ number_format($payment->amount) }} ؋</td>
        </tr>
        <tr>
            <td class="label">باقی‌مانده بدهی</td>
            <td class="value">{{ number_format($balance) }} ؋</td>
        </tr>
    </table>
    ...
    <div class="footer">
        تاریخ چاپ: @jalali(now()) | سامانه مدیریت مالی الزهرا (س)
    </div>
</div>
</body>
</html>
