...
<div class="no-print" style="display: flex; justify-content: space-between; align-items: center;">
    ...
    <form method="GET" action="{{ route('school.reports.comprehensive') }}" class="filter-form">
        <label>ماه:</label>
        <select name="month_id" onchange="this.form.submit()">
            <option value="">همه ماه‌ها</option>
            @foreach($months as $m)
                <option value="{{ $m->id }}" {{ $monthId == $m->id ? 'selected' : '' }}>{{ $m->name }}</option>
            @endforeach
        </select>
    </form>
</div>

{{-- کادر عنوان + لوگو + سال مالی --}}
@php
...
">
    ...
    <div style="flex: 1; text-align: center;">
        ...
        <h4 style="margin: 5px 0 0 0; font-size: 14px; font-weight: normal; color: #555;">
            {{ $selectedMonth ? 'ماه: ' . $selectedMonth->name : 'کل سال' }}
        </h4>

    </div>

    {{-- فضای خالی برای تقارن --}}
    <div style="flex: 0 0 auto; width: 70px; margin-right: 20px;"></div>
</div>

<!-- ======================== ۱. ورودی‌ها (درآمدها) ======================== -->
<h3>الف) ورودی‌ها – ماه {{ $selectedMonth->name ?? 'جاری' }}</h3>
...
<table>
    <thead>
        <tr>
    ...
    <tbody>
        @foreach($feePaymentsGrouped as $group)
            <tr>
                <td>شهریه دانش آموزان</td>
                <td>{{ $group['count'] }}</td>
                <td>{{ number_format($group['amount']) }}</td>
        ...
        <tr>
            ...
            <td class="bold">{{ number_format($otherIncomes) }}</td>
            <td></td>
        </tr>
        <tr>
            <td class="bold" colspan="4">کمک هزینه دریافتی</td>
            <td class="bold">{{ number_format($grants) }}</td>
            <td></td>
        </tr>
    </tbody>
    <tfoot>
        <tr>
            ...
            <td class="bold">{{ number_format($totalIncome) }}</td>
            <td></td>
        </tr>
    </tfoot>
</table>
...
<table>
    ...
    <tbody>
        ...
        @foreach($expensesGrouped as $group)
...
<table>
    ...
    <tbody>
        ...
        @foreach($salariesQuery as $salary)
            @php $salaryRunningTotal += ($salary->paid_amount ?? 0); @endphp
            <tr>
                <td>{{ $salary->id }}</td>
                ...
                <td>{{ number_format($salary->total_amount) }}</td>
                <td>{{ number_format($salary->overtime_amount ?? 0) }}</td>
                <td>{{ number_format(($salary->deduction_amount ?? 0) + ($salary->tax_amount ?? 0)) }}</td>
    ...
    <tfoot>
        <tr class="summary-table">
            ...
            <td>{{ number_format($salariesQuery->sum('deduction_amount') + $salariesQuery->sum('tax_amount')) }}</td>
...
<h3>د) خلاصه مالی</h3>
<table>
    <tr><td width="30%">درآمد مستقل</td><td>{{ number_format($otherIncomes) }} ؋</td></tr>
    <tr><td>کمک هزینه ها</td><td>{{ number_format($grants) }} ؋</td></tr>
    <tr><td>مجموع درآمدها</td><td>{{ number_format($totalIncome) }} ؋</td></tr>
    <tr><td>مصارف</td><td>{{ number_format($totalExpenses) }} ؋</td></tr>
    <tr><td>معاشات</td><td>{{ number_format($totalSalaries) }} ؋</td></tr>
    <tr><td>مجموع مصارف</td><td>{{ number_format($totalExpenses + $totalSalaries) }} ؋</td></tr>
    <tr><td>کسری/مازاد</td><td>{{ number_format($totalIncome - ($totalExpenses + $totalSalaries)) }} ؋</td></tr>
    <tr><td>موجودی صندوق</td><td>{{ number_format($cashboxBalance) }} ؋</td></tr>
</table>

<p style="text-align:center; margin-top:20px;">گزارش جامع تولید شده توسط سامانه مدیریت مالی الزهرا (س) – {{ \App\Helpers\JalaliHelper::todayJalali() }}</p>
</body>
</html>
