...
<div class="receipt-container">
    ...
    <div class="header">
        ...
        <div>{{ $installment->loan->loan_provider ?? $installment->loan->school->name ?? 'صندوق قرض‌الحسنه' }}</div>
        <h4>رسید پرداخت قسط وام</h4>
    </div>

    <table class="info-table">
        <tr><td class="label">قرض‌گیرنده</td><td>{{ $installment->loan->borrower_name }} {{ $installment->loan->borrower_last_name }}</td></tr>
        <tr><td class="label">شماره قسط</td><td>{{ $installment->loan->installments->search($installment) + 1 }} از {{ $installment->loan->duration_months }}</td></tr>
        <tr><td class="label">مبلغ پرداختی</td><td>{{ number_format($installment->paid_amount) }} ؋</td></tr>
        <tr><td class="label">تاریخ پرداخت</td><td>{{ \App\Helpers\JalaliHelper::toJalali($installment->paid_date) }}</td></tr>
        <tr><td class="label">صندوق</td><td>{{ $installment->cashbox->name ?? '—' }}</td></tr>
    </table>
    ...
    <div class="footer">تاریخ چاپ: {{ \App\Helpers\JalaliHelper::todayJalali() }}</div>
</div>
</body>
</html>
